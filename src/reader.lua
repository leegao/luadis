...
local utils = {int=contexualize(int), short=contexualize(short), byte=contexualize(byte), string=contexualize(string), double=contexualize(double), contexualize = contexualize}

local function new_ctx(str)
	return setmetatable({str, 1}, {__index=utils})
end

utils.new_ctx = new_ctx
